...
func GetById(id string) (ad Ad, err error) {
	...
	row := conn.QueryRow(`SELECT * FROM ads WHERE dateend >= current_date and status=0 and id=$1`, id)
...
func Get(status string) (ads []Ad, err error) {
	...
	rows, err := conn.Query(`SELECT * FROM ads WHERE dateend >= current_date and status=$1`, status)
